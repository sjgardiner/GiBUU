...
contains
  ...
  subroutine read_fluxfile(NDIM,fluxfilename,n_E,enu,flux,sumflux)
    ...
    !**************************************************************************
    !****n* esample/nl_fluxcuts
    ! NAME
    ! NAMELIST /nl_fluxcuts/
    ! PURPOSE
   ...
   ! read in possible flux cuts:
    ...
    call Write_ReadingInput('nl_fluxcuts',1)

    ! reading of flux file from buuinput/neutrinos:
    if (index(fluxfilename,"/")>0) then
       filename=trim(fluxfilename) ! already absolute path
    else
