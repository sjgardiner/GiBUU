...
!!$    if (DebugFlag) then
!!$       if (part%ID==101) then
!!$          write(124,'(1P,4e15.7)') part%pos
!!$          write(124,'(1P,4e15.7)') part%mom
!!$          mm = part%mom
...
!!$    end if
    ...
    if (isMeson(part%ID) &
         .and. treatParticleOffShell(part%ID,part%offshellPar) &
         .and. checkE &
    ...
    if (checkE .and. abs(part%mom(0)-E0)>delta_E) then
       write(*,*) "WARNING in EnergyDetermination: Energy not conserved!",part%mom(0)-E0
       write(*,*) 'E0 = ', E0
...
!       call plot_f(part)
  ...
  contains
    ...
    subroutine MesonHistFailures(iTyp)
      ...
      case (1)
         ...
         select case (part%ID)
         case (rho)
            call AddHist2D(hist2D_rho,(/abs4(part%mom),absMom(part)/),0.,1.)
         case (omegaMeson)
            call AddHist2D(hist2D_omega,(/abs4(part%mom),absMom(part)/),0.,1.)
         case (phi)
            call AddHist2D(hist2D_phi,(/abs4(part%mom),absMom(part)/),0.,1.)
         end select
      ...
      case (2)
         ...
         select case (part%ID)
         case (rho)
            call AddHist2D(hist2D_rho,(/abs4(part%mom),absMom(part)/),1.)
         case (omegaMeson)
            call AddHist2D(hist2D_omega,(/abs4(part%mom),absMom(part)/),1.)
         case (phi)
            call AddHist2D(hist2D_phi,(/abs4(part%mom),absMom(part)/),1.)
         end select
    ...
    logical function vary_mass_offshell()
      ...
      do
         part2%mom(0)=part2%mom(0)+delta_pNull
         error=m_eff_sqr(part2,density)-(part2%mom(0)**2-psqr)
    ...
    subroutine plot_f(part_in)
      ...
      do i=0,max ! Loop which steps through the energy
         ...
         !if(i.eq.0) old=resu
         write(111,'(4E15.7)') part2%mom(0),resu,me2,ener
    ...
    subroutine errorMessage
      ...
      write(*,'(a,4E12.4)')'Momentum of particle :', part%mom
      write(*,'(a,E12.4)') 'Abs. mom. of particle:', absMom(part)
      write(*,'(a,L2)')    'Perturbative         :', part%pert
      write(*,'(a,E12.4)') 'Offshell parameter   :', part%offshellPar
      if (present(betaToCF)) then
    ...
    real function m_eff_sqr(partIn, density, enerOut)
      ...
      if (CFBoostFlag) call lorentz(betaToCF,part%mom)
      ...
      if (treatParticleOffShell(part%ID,part%offshellPar)) then
         ...
         outOfBounds_offshell=.false.
         if (.not.success .or. SP(part%mom,part%mom)<0.) then
            ! BAD Solution !!
            part%mom(0)=999999999.
         else
            part%mom(0)=HamiltonFunc_offshell(part,outOfBounds_offshell,.true.)
         end if
      else
         ! (1.3) Boost from calculation frame to LRF
         call boostToLRF(part,1,density)
         part%mom(0)=FreeEnergy(part)+potential_LRF(part,density,addCoulomb=.false.)
      end if
      ...
      if (DoCoulomb) then
         ...
         part%mom(0)=part%mom(0) &
              + emfoca(part%pos,part%mom(1:3),part%charge,part%ID)
      end if


      if (present(enerOut)) enerOUT=part%mom(0)

      m_eff_sqr=part%mom(0)**2-Dot_product(part%mom(1:3),part%mom(1:3))
  ...
  ! NAME
  ! subroutine energyCorrection(srqts, betaToCM,
  ...
  !****************************************************************************
  subroutine energyCorrection(srts, betaToCM, &
       & finalState, successFlag, potentialFailure, verbose)
    ...
    use random, only: rn
    use mesonPotentialMain, only: getNoPertPot_meson
    use baryonPotentialMain, only: getNoPertPot_baryon
    use RMF, only: getRMF_flag

    real, intent(in) :: srtS
    real, intent(in), dimension(1:3) :: betaToCM
    ...
    if (.not.getRMF_flag()) then
       if (getNoPertPot_meson().and.getNoPertPot_baryon()) then
          ...
          if (finalState(1)%pert.and.(Sum(finalstate%mass)>srts)) then
  ...
  contains
    ...
    subroutine setFinalState
      integer :: i
      do i=1,size(part,dim=1)
         call lorentz(-betaToCM,part(i)%mom(0:3))  ! Boost to Calc frame
      end do
      finalState = part
    ...
    real function deltaSrts(x)
      ...
      do i=1, size( finalState, dim=1 )
         part(i)%mom(1:3)=x * part(i)%mom(1:3)
      end do

      if (debug) write(*,*) 'moms vorher =' , &
           & sum(part%mom(0)),sum(part%mom(1)), &
           & sum(part%mom(2)),sum(part%mom(3))

      do i=1, size( finalState, dim=1 )
         if (debug) write(*,*) 'part ',i,'vorher :',part(i)%mass, part(i)%mom,part(i)%pert
         if ( .not.getRMF_flag() ) then
            call energyDetermination(part(i),-betaToCM,skipTestE0=.true.)
         else
            call energyDeterminationRMF(part(i))
         end if
         if (debug) write(*,*) 'part ',i,'nachher :',part(i)%mass, part(i)%mom
      end do

      deltaSrts=srts-Sum(part%mom(0))
